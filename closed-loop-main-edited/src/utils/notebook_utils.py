...
def add_unirep_model(df: pd.DataFrame, model_names: List[str], path: str, name: str) -> tuple[pd.DataFrame, List[str]]:
...
def add_ev_model(df: pd.DataFrame, model_names: List[str], path: str, name: str, dataset: str, include_indep: bool=False) -> tuple[pd.DataFrame, List[str]]:
...
def add_hmm_model(df: pd.DataFrame, model_names: List[str], path: str, name: str, dataset: str) -> tuple[pd.DataFrame, List[str]]:
    df = df.sort_values('seq')
...
def retrieve_metric(df: pd.DataFrame, metric_name: str, n_mut: Optional[int]=None, predictor: Optional[Union[str, Sequence[str]]]=None) -> pd.DataFrame:
    tmp = df
...
def metric_lineplot(df: pd.DataFrame, predictors: Sequence[str], metric: str, predictor_names: Sequence[str], dataset_name: str,
    max_n_mut: int, savename: str='figure', legend: Optional[Dict[str, Any]]=None, mutcounts: Optional[Sequence[float]]=None, **kwargs) -> None:
    ...
    ax.set_ylabel(metric_display_name.get(metric, metric))
