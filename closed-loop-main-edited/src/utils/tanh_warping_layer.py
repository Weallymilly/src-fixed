...
class TanhWarpingLayer(torch.nn.Module):
    ...
    """
    def __init__(self, num_warping_terms: int):
    ...
    def set_parameters(self, a: list[float], b: list[float], c: list[float], d: float) -> None:
        ...
        if len(a) != self.num_warping_terms or len(b) != self.num_warping_terms or (
            ...
            raise ValueError(f"Expected {self.num_warping_terms} warping terms")
        self.pre_d.data.fill_(inv_logexp(d).item())
        for i, t in enumerate(self.warping_terms):
            t.set_parameters(a[i], b[i], c[i])

    def get_parameters(self) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
