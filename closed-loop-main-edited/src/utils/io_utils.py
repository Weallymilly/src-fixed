...
def load_data_split(dataset_name, split_id, seed=0, ignore_gaps=False):
    ...
    data_pre_split = pd.read_csv(data_path)
    if not ignore_gaps:
...
def load_data_test(dataset_name, split_id, seed=0, ignore_gaps=False):
    data_path = os.path.join('data', dataset_name, 'test-data.csv')
    # Sample shuffles the DataFrame.
    # data_pre_split = pd.read_csv(data_path).sample(frac=1.0, random_state=seed)
    data_pre_split = pd.read_csv(data_path)
    if not ignore_gaps:
...
def load(filename_glob_pattern):
    ...
    if not files:
        raise FileNotFoundError(f"No files found for pattern {filename_glob_pattern}")
